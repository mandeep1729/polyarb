...
class TestCleanup:
    """Test simplified cleanup_old_snapshots."""

    @pytest.mark.asyncio
    async def test_cleanup_deletes_resolved_market_snapshots(
        self, db: AsyncSession, platform: Platform
    ):
        """Snapshots for old resolved markets should be deleted."""
        old_market = UnifiedMarket(
            platform_id=platform.id,
            platform_market_id="RESOLVED-OLD",
        ...
        await db.execute(
            text(
                "UPDATE unified_markets SET updated_at = :ts WHERE id = :mid"
            ).bindparams(
        ...
        assert old_market.id in resolved_ids

        await db.execute(
            delete(PriceSnapshot).where(PriceSnapshot.market_id.in_(resolved_ids))
        )

        result = await db.execute(
            select(PriceSnapshot).where(PriceSnapshot.market_id == old_market.id)
        )
        assert len(result.scalars().all()) == 0

    @pytest.mark.asyncio
    async def test_cleanup_preserves_active_market_snapshots(
        self, db: AsyncSession, market: UnifiedMarket
    ):
        """Snapshots for active markets should NOT be deleted by cleanup."""
        snap = PriceSnapshot(
            market_id=market.id,
            outcome_prices={"Yes": 0.5},
        ...
        result = await db.execute(
            select(PriceSnapshot).where(PriceSnapshot.market_id == market.id)
        )
